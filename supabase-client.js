(function(){
  ...
  function getSessionId(){let id=localStorage.getItem(sessionKey);if(!id){id=crypto.randomUUID?crypto.randomUUID():`${Date.now()}-${Math.random().toString(36).slice(2)}`;localStorage.setItem(sessionKey,id)}return id}
